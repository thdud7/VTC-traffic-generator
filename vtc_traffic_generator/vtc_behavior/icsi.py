...
class ICSIReplayPolicy:
    ...
    def __init__(
        ...
        self.dialogue_acts_dir = self.corpus_root / "DialogueActs"
        ...
        if not self.dialogue_acts_dir.exists():
            raise FileNotFoundError(f"Missing ICSI DialogueActs directory: {self.dialogue_acts_dir}")
    ...
    def from_config(cls, config: Mapping[str, Any], bot_count: int) -> "ICSIReplayPolicy":
        ...
        if not corpus_root:
            raise ValueError("ICSI mode requires behavior.icsi.corpus_root.")
